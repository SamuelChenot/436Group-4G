...
def updateDatabase(mountainInfo, time):
    ...
    mountainNames.sort()
    #-----------------------
    for i in range(mountainNames):
        #[...            ,(Name          , MID )]=> As Tuple
        mountainNames[i] = (mountainNames[i], i)
   
    
    #Hourly csv
    file = open("hourly.csv", "w")
    ...
    for name in mountainNames:
        ...
        for h_temp, h_apparentTemp, h_precipIntensity, h_precipProb, \
        h_precipType, h_windSpeed, h_windGust, h_windBearing, h_humids, \
        h_clouds, h_vis ,h_snow in mountainInfo[name[0]]["hourly"].items():
        """

        h_data = mountainInfo[name[0]]["hourly"].values()
        h_precipIntensity = h_data[2]
...
def main():
    ...
    while False:
        ...
        if(curr_time == "12:00:00" or curr_time == "00:00:00"):
            mountainInfo = assembleData(mountains, curr_time)
